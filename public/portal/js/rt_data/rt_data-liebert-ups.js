$(document).ready(function() {
	var dataIdArr9 = new Array();//liebert ups
	$('.rt-data').each(function() {
		if ($(this).attr('data_type') == 'liebert-ups' && dataIdArr9.indexOf($(this).attr('data_id')) == -1) {
	...
	function refreshData() {
		$.get('/portal/refreshData', {
			...
			for(var i = 0 ; i < ret.libertUpsList.length ; i++)
			{
				var obj = ret.libertUpsList[i];
				...
				$('#liebert-ups-' + obj.data_id + ' .status').html(obj.isEmpty ? '离线': '在线');
				$('#liebert-ups-' + obj.data_id + ' .update_time').html(obj.update_datetime);
				if(obj.isEmpty){
					$('#liebert-ups-' + obj.data_id + ' table:eq(1) tr').each(function(){
						$('span:eq(0)',this).addClass('text-error lead');
						$('span:eq(0)',this).html("暂无数据");
						$('span:eq(1)',this).html("无状态");
					});
					$('#liebert-ups-' + obj.data_id + ' table:eq(2) tr').each(function(){
						$('span:eq(0)',this).addClass('text-error lead');
						$('span:eq(0)',this).html("暂无数据");
						$('span:eq(1)',this).html("无状态");
					});
					...
					$('#liebert-ups-' + obj.data_id + ' table:eq(1) tr td>span').attr('class','');
					$('#liebert-ups-' + obj.data_id + ' table:eq(2) tr td>span').attr('class','');
				}					
				
				for(var k = 0 ; k < obj.aList.length ; k++)
				{
					var aObj = obj.aList[k];
					$('#liebert-ups-' + obj.data_id + ' table:eq(1)>tbody>tr:eq('+ k +') td>span:eq(0)').html(aObj.val);
					var levelStr = '正常';
					var cls = '';
					...
					$('#liebert-ups-' + obj.data_id + ' table:eq(1)>tbody>tr:eq('+ k +') td>span:eq(1)').html(levelStr);
					$('#liebert-ups-' + obj.data_id + ' table:eq(1)>tbody>tr:eq('+ k +') td>span:eq(1)').attr('class',cls);
				}
				...
				for(var j = 0 ; j < obj.d1List.length ; j++)
				{
					...
					msg += "&nbsp;&nbsp;&nbsp;&nbsp;（原始值:" + d1Obj.val +"）";
					$('#liebert-ups-' + obj.data_id + '-field' + j +' span:eq(0)').html(msg);
					$('#liebert-ups-' + obj.data_id + '-field' + j +' span:eq(0)').attr('class',cls);
					...
					$('#liebert-ups-' + obj.data_id + '-field' + j +' span:eq(1)').html(levelStr);
					$('#liebert-ups-' + obj.data_id + '-field' + j +' span:eq(1)').attr('class',cls);
				}
			}
		});
	}
