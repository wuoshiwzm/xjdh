$(document).ready(function() {
	...
	$('.rChoose').change(function(){
		radioObj = $(this);
		$('#txtDcScript').insert({"text":radioObj.attr('perfortype')});
	});
	...
	$(".btn-del").click(function(){
		if(confirm("请确认删除此动态配置?"))
		{
			...
			$.post("/portal/deletePerforConfig", { substation_id:substation_id}, function(ret){
				if(ret)
				{
					pTr.remove();
					var n = noty({
	...
	$('#btn-save').click(function(){
		$.post('/portal/savePerforConfig',{substation_id:substation_id,nk_script:$('#txtDcScript').val()},
			function(ret){
			if(ret){
