$(document).ready(function() {
	...
	$(".setting").click(function(){
		...
		$('#network').empty();
		$.post("/portal/get_perfor_settings", {id:pTr.attr("id"),substation_id:pTr.attr("substation_id")},function(data){
			eval("var ret = " + data);
	...
	$('#area-tree').bind("activate_node.jstree", function (event,data) {
	  var substation_id = $('#area-tree').jstree().get_bottom_selected();
    	  ...
    	  substation_id = substation_id.replace("[\"##",""); 
 	      substation_id = substation_id.replace("\"]",""); 
 	      substation_id = substation_id.replace("[\"",""); 
 	      window.location.href="/portal/substation_performance_manage/"+substation_id; 
	});
